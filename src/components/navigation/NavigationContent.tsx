...
const NavigationContent = ({ isBlog }: NavigationContentProps) => {
  return (
    <nav className="sticky max-w-5xl xl:max-w-6xl top-0 text-foreground w-full md:px-4 md:py-4 transition-all ease-in-out duration-300 z-50 mx-auto">
      <div className="glass flex gap-6 w-full justify-start items-center mx-auto px-4 py-3 md:rounded-xl shadow-sm overflow-hidden">
        <Link
          ...
          prefetch={false}>
          <small
            className={` ${isBlog ? "border-blue-600 text-blue-500" : "border-arbxz-accent text-arbxz-accent"} text-sm rounded-full px-4 py-1 border-2 font-semibold transition-colors duration-300`}>
            Arbxz
          </small>
        </Link>
